...
namespace DiamDev.Give.UI.Controllers
{
    ...
    {
        private const int MaxRangeDays = 1096;
        ...
        public JsonResult GetData(string from, string to, string empresa)
        {
            ...
            if (!TryParseDateRange(from, to, out fromDate, out toDate))
            {
                return JsonError(
                    400,
                    "Las fechas deben usar el formato yyyy-MM-dd, el inicio no puede ser posterior al fin y el rango máximo es de 1096 días.");
            }
            ...
            try
            {
                var connectionSetting =
                    ConfigurationManager.ConnectionStrings["HanaOdbc"];
                if (connectionSetting == null ||
                    string.IsNullOrWhiteSpace(connectionSetting.ConnectionString))
                {
                ...
                var sqlPath = Server.MapPath("~/App_Data/backlog.sql");
                if (string.IsNullOrWhiteSpace(sqlPath) ||
                    !System.IO.File.Exists(sqlPath))
                {
                    throw new FileNotFoundException(
                        "No se encontró la consulta del dashboard de backlog.",
                        sqlPath);
                }
            ...
            {
                Trace.TraceError(
                    "Backlog.GetData falló para empresa {0}, rango {1:yyyy-MM-dd}..{2:yyyy-MM-dd}: {3}",
                    empresa,
                    fromDate,
                    toDate,
                    ex);
                ...
                return JsonError(
                    500,
                    "No fue posible consultar la información del backlog. Intenta nuevamente o contacta a IT.");
            }
        }
            ...
            DateTime toDate)
        {
            ...
            using (var connection = new OdbcConnection(connectionString))
            {
                ...
                using (var command = new OdbcCommand(sql, connection))
                {
                    ...
                    // con los dos signos ? de backlog.sql.
