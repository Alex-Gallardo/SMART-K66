...
const styles = read("DiamDev.Give.UI/Content/borrador-nc-dashboard.css");
const script = read("DiamDev.Give.UI/Scripts/App/BorradorNc-Dashboard.js");
...
assert.match(dal, /;WITH Antecedentes AS/);
assert.match(dal, /LEFT JOIN Adjuntos J/);
assert.doesNotMatch(dal, /SUM\s*\(\s*CASE\s+WHEN\s+EXISTS/i);
assert.match(view, /Exportar Excel/);
assert.match(view, /Imprimir selección/);
assert.match(view, /data-url-bitacora/);
assert.match(view, /id="filtrosAvanzados" hidden/);
assert.match(view, /aria-live="polite"/);
assert.match(script, /No pudimos cargar el Dashboard/);
assert.match(script, /No encontramos borradores/);
assert.match(script, /solicitudActual\.abort\(\)/);
assert.match(script, /aria-sort/);
assert.match(styles, /@media \(max-width: 767px\)/);
assert.match(styles, /@media \(prefers-reduced-motion: reduce\)/);
assert.match(styles, /\.bncd-table tbody tr \{[^}]*display: grid/s);
